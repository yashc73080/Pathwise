        ...
        <nav className={`md:hidden fixed bottom-0 left-0 right-0 z-50 px-3 flex items-center gap-2 bg-gradient-to-t from-white via-white to-transparent transition-all duration-300 ${isFullPanelOpen ? 'pb-2 pt-1' : 'pb-4 pt-2 gap-3'}`}>
            ...
            <div className={`flex flex-1 bg-white shadow-lg overflow-hidden transition-all duration-300 ${isFullPanelOpen ? 'rounded-xl' : 'rounded-2xl'}`}>
                ...
                <button
                    ...
                    className={`flex-1 flex flex-col items-center justify-center transition-all duration-300 ${isFullPanelOpen ? 'py-1.5' : 'py-3'} ${activePanel === 'itinerary'
                ...
                >
                    ...
                    {!isFullPanelOpen && <span className="text-[10px] mt-0.5 font-medium">Trip</span>}
                </button>

                {/* Divider */}
                <div className={`w-px bg-gray-200 transition-all duration-300 ${isFullPanelOpen ? 'my-1.5' : 'my-2'}`}></div>

                {/* Route Tab */}
                <button
                    onClick={() => handleTabClick('route')}
                    className={`flex-1 flex flex-col items-center justify-center transition-all duration-300 ${isFullPanelOpen ? 'py-1.5' : 'py-3'} ${activePanel === 'route'
                        ? 'text-blue-600 bg-blue-50'
                        : optimizedRoute
                            ? 'text-gray-500'
                            : 'text-gray-300'
                ...
                >
                    ...
                    {!isFullPanelOpen && <span className="text-[10px] mt-0.5 font-medium">Route</span>}
                </button>
                ...
                <button
                    ...
                    className={`flex-1 flex flex-col items-center justify-center transition-all duration-300 ${isFullPanelOpen ? 'py-1.5' : 'py-3'} ${activePanel === 'chat'
                        ? 'text-blue-600 bg-blue-50'
                        : 'text-gray-500'
                        }`}
                ...
                >
                    ...
                    {!isFullPanelOpen && <span className="text-[10px] mt-0.5 font-medium">AI</span>}
                </button>
            </div>
        </nav>
    );
