...
// Blue itinerary marker; shows a numbered circle when a route position is known,
// otherwise the classic blue-dot pin
export function createItineraryMarker({ map, position, title, number = null }) {
    const { AdvancedMarkerElement } = window.google.maps.marker;
    return new AdvancedMarkerElement({
        map,
        position,
        title,
        content: number ? numberedCircle(number) : blueDot(),
    });
}

// Swap an itinerary marker's content to a numbered route circle
export function setMarkerNumber(marker, number) {
    marker.content = numberedCircle(number);
}
...
function numberedCircle(number) {
    ...
    el.style.cssText =
        'width:24px;height:24px;border-radius:50%;background:#2563eb;border:2px solid #1e40af;' +
        'color:white;font-size:12px;font-weight:bold;display:flex;align-items:center;' +
